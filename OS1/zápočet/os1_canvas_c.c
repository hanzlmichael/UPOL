...
{
	int countSymbolsOnRow = 0; // poèítadlo printnutych symbolu na 1 radku
	...
	for (int i = 0; i < canvas->size; i++)
	{
		...
		for (int j = 0; j < 8; j++)
		{
			...
			if (countSymbolsOnRow == canvas->width) // pokud je na radku stejny pocet jako sirka canvas
			{										// tak vytisknu novy radek a vynuluju counter
				printf("\n"); 
				countSymbolsOnRow = 0;
			}
